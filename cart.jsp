		...
		<tbody>
			...
			<tr>
				<td>
					...
					//Get Cart
					sqlQuery = "SELECT * FROM cart WHERE username = '" + username + "' ORDER BY furnitureid DESC LIMIT 1";
					preparedStmt = connectionDB.prepareStatement(sqlQuery);
					result = preparedStmt.executeQuery();
					while (result.next()) {
					%>
						...
						<table class="tab_cart" style="display: table">
							...
							<tbody>
								<%
									...
									bookID = result.getInt("furnitureid");
									qty = result.getInt("qty");

									//Get Book Name
									sqlQuery = "SELECT name FROM furnitures WHERE Bid = '" + bookID + "' ORDER BY Bid";
									preparedStmt = connectionDB.prepareStatement(sqlQuery);
									result = preparedStmt.executeQuery();
									...
									//Get Book price
									sqlQuery = "SELECT price FROM furnitures WHERE Bid = '" + bookID + "' ORDER BY Bid";
									preparedStmt = connectionDB.prepareStatement(sqlQuery);
									result = preparedStmt.executeQuery();
									while (result.next()) {
